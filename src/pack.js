define(function(require){
    ...
    return function($p, dataProps) {
        ...
        if ($p.indexes.length === 0) {
            $p.attribute("aIndex0", "float", utils.seqFloat(0, $p.dataDimension[0] - 1));
            $p.attribute("aIndex1", "float", utils.seqFloat(0, $p.dataDimension[1] - 1));
            $p.attribute("aIndex0Value", "float", utils.seqFloat(0, $p.dataDimension[0] - 1));
            $p.attribute("aIndex1Value", "float", utils.seqFloat(0, $p.dataDimension[1] - 1));
        } else {
            $p.indexes.forEach(function(id, i) {
                var indexAttrData = arrays.unique(data[id]).sort(function(a, b) {
                    return a - b;
                });
                $p.attribute("aIndex" + i + "Value", "float", new Float32Array(indexAttrData));
                $p.attribute("aIndex" + i, "float", utils.seqFloat(0, indexAttrData.length - 1));
                $p.fieldWidths[i] = indexAttrData.length;
                $p.dataDimension[i] = indexAttrData.length;
            });
        }

        $p.attribute("_vid", "float", utils.seqFloat(0, $p.dataDimension[0] * $p.dataDimension[1] - 1));
        $p.attribute("_fid", "vec2", utils.seqFloat(0, $p.fields.length * 2));
        $p.attribute("_qid", "float", [0, 1, 2, 3, 4, 5]);
        $p.ctx.ext.vertexAttribDivisorANGLE($p.attribute._qid.location, 0);
        $p.ctx.ext.vertexAttribDivisorANGLE($p.attribute._fid.location, 0);
        $p.ctx.ext.vertexAttribDivisorANGLE($p.attribute._vid.location, 1);
        ...
        $p.varying("vResult", "float")
            ...
            .framebuffer("fFilterResults", "unsigned_byte", $p.dataDimension)
            .framebuffer("fGroupResults", "float", [1024, 1])
            .framebuffer("fDerivedValues", "float", [$p.dataDimension[0], $p.dataDimension[1] * $p.deriveMax]);
        ...
        var gl = $p.ctx;
        gl.ext.vertexAttribDivisorANGLE($p.attribute.aIndex0.location, 0);
        gl.ext.vertexAttribDivisorANGLE($p.attribute.aIndex0Value.location, 0);
        gl.ext.vertexAttribDivisorANGLE($p.attribute.aIndex1.location, 1);
        gl.ext.vertexAttribDivisorANGLE($p.attribute.aIndex1Value.location, 1);
